...
export function resolveSource(env: NodeJS.ProcessEnv): ResolveSourceResult {
  ...
  if (requested !== undefined && requested !== "") {
    ...
    return { source: createSource(env, alias, apiKey) };
  }

  for (const alias of DETECTION_ORDER) {
    const apiKey = firstEnv(env, PROVIDERS[alias].keyVars);
    if (apiKey !== undefined) return { source: createSource(env, alias, apiKey) };
  }
